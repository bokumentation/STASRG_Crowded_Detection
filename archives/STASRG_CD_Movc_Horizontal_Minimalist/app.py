...
print("Versi: Horizontal Line | Versi: 1.0.0")
print("Tunggu...")

app = Flask(__name__, static_folder='static')

entry_line_position = 320 # Ori: 320
exit_line_position = 160 # Ori: 160
entry_count = 0
exit_count = 0
...
def generate_frames():
    ...
    while True:
        ...
        results = model(frame, verbose=False) # added verbose=False - Ibe
        rects = []
        ...
        dalam_count = entry_count - exit_count # Note: Unused variable -ibe
...
def open_browser():
    ...
    time.sleep(2) # Wait for the Flask server and the CV thread to start up
    ...
    for browser_path in browser_paths:
        if os.path.exists(browser_path):
            try:
                subprocess.Popen([browser_path, f"--app={app_url}", "--start-fullscreen"])
