...
# KODINGAN ADIB
entry_line_position = 320  # asli dr adib: 320; G Line
exit_line_position = 160  # asli dr adib: 160; R Line
...
def detect_direction(object_id, cy, prev_cy):
    ...
    if prev_cy is not None:
        if prev_cy < cy:
            # Bergerak melewati exit line (ke bawah)
            if prev_cy < entry_line_position <= cy:
                if (
                    object_id not in counted_on_exit
                ):  # Hanya hitung jika belum dihitung sebelumnya
                    exit_count += 1
                    # Tandai objek sebagai dihitung di exit
                    counted_on_exit.add(object_id)
                    if (
                        object_id in counted_on_entry
                    ):  # Hapus dari entry jika berpindah garis
                        counted_on_entry.remove(object_id)
            return "Exit"
        elif prev_cy > cy:
            # Bergerak melewati entry line (ke atas)
            if prev_cy > exit_line_position >= cy:
                if (
                    object_id not in counted_on_entry
                ):  # Hanya hitung jika belum dihitung sebelumnya
                    entry_count += 1
                    # Tandai objek sebagai dihitung di entry
                    counted_on_entry.add(object_id)
                    if (
                        object_id in counted_on_exit
                    ):  # Hapus dari exit jika berpindah garis
                        counted_on_exit.remove(object_id)
            return "Entry"

    if (
        prev_cy is not None
        and abs(cy - entry_line_position) > 100
        and object_id in counted_on_entry
    ):
        counted_on_entry.discard(object_id)
    if (
        prev_cy is not None
        and abs(cy - exit_line_position) > 100
        and object_id in counted_on_exit
    ):
...
def generate_frames():
    ...
    while True:
        ...
        cv2.line(
            frame,
            (0, entry_line_position),
            (frame.shape[1], entry_line_position),
            (0, 100, 0),
            2,
        )
        cv2.line(
            frame,
            (0, exit_line_position),
            (frame.shape[1], exit_line_position),
            (0, 0, 255),
            2,
        )
...
graph_data = {"time_labels": [], "count_data": []}
...
def download_excel():
    ...
    for item in local_log_data:
        ws.append([item["time"], item["current"], item["entry"], item["exit"]])
